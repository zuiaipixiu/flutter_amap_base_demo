...
static NSString *mapMoveChannelName = @"me.yohom/map_moved";
...
@interface MapMoveEventHandler : NSObject <FlutterStreamHandler>
@property(nonatomic) FlutterEventSink moveSink;

@end

@implementation MapMoveEventHandler {
...
- (void)setup {
  ...
  _eventHandler = [[MarkerEventHandler alloc] init];
    ...
    _moveEventHandle = [[MapMoveEventHandler alloc] init];
    _mapMoveEventChannel = [FlutterEventChannel eventChannelWithName:[NSString stringWithFormat:@"%@%lld", mapMoveChannelName, _viewId] binaryMessenger:[AMapBasePlugin registrar].messenger];
    [_mapMoveEventChannel setStreamHandler:_moveEventHandle];
}
    ...
    if ([overlay isKindOfClass:[CircleOverlay class]])
     {
         ...
         circleRenderer.lineWidth    = (CGFloat) options.strokeWidth;
...
- (MAAnnotationView *)mapView:(MAMapView *)mapView viewForAnnotation:(id <MAAnnotation>)annotation {
  ...
  if ([annotation isKindOfClass:[MAPointAnnotation class]]) {
    ...
    static NSString *routePlanningCellIdentifier = @"CustomCalloutAnnotationView";

    CustomCalloutAnnotationView *annotationView = [_mapView dequeueReusableAnnotationViewWithIdentifier:routePlanningCellIdentifier];
    if (annotationView == nil) {
      annotationView = [[CustomCalloutAnnotationView alloc] initWithAnnotation:annotation
    ...
    if ([annotation isKindOfClass:[MarkerAnnotation class]]) {
      ...
      annotationView.calloutView.hidden = !options.haveBubble;
      annotationView.addressLabel.hidden = !options.haveAddressLabel;
        [annotationView updateCalloutInfoWithContent:options.bubbleContent];
        [annotationView updateAddresCalloutWithContent:options.addressLabelContent];
        
        
        if ([[annotation title] isEqualToString:@"start"]) {
            [MarkTool shareInstance].startAnnotation = annotationView;
        } else if ([[annotation title] isEqualToString:@"end"]) {
            [MarkTool shareInstance].endAnnotation = annotationView;
        } else if ([[annotation title] isEqualToString:@"car"]) {
            [MarkTool shareInstance].carAnnotation = annotationView;
        }
...
//    NSDictionary *dic = @{@"event":@"mapMove",
//                          @"status":@"stop",
//    };
//    DLog(@"%@",dic);
    
//    DLog(@"判断是否初始化 %@, %@", _moveEventHandle,_moveEventHandle.moveSink);
    
    if (_moveEventHandle && _moveEventHandle.moveSink) {
        _moveEventHandle.moveSink(@"stop");
    }
