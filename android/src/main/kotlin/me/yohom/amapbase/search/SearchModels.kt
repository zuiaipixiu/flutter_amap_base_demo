...
//region UnifiedRideRouteResult
internal class UnifiedRideRouteResult(rideRouteResult: RideRouteResult) {
    /// 起始
    val startPos: LatLng = rideRouteResult.startPos.toLatLng()

    /// 终点
    val targetPos: LatLng = rideRouteResult.targetPos.toLatLng()



    /// 路段
    val paths: List<UnifiedRidePath> = rideRouteResult.paths.map { UnifiedRidePath(it) }
}
class UnifiedRidePath(ridePath: RidePath) {
    val totalDistance:Int = ridePath.distance.toInt()
    val totalDuration: Long = ridePath.duration
}
...
class UnifiedDrivePath(drivePath: DrivePath) {
    ...
    val totalDistance:Int = drivePath.distance.toInt()
    val totalDuration: Long = drivePath.duration
}
...
class UnifiedSearchBound(searchBound: PoiSearch.SearchBound) {
    ...
    fun toSearchBoundCenterRange(): PoiSearch.SearchBound {
        return PoiSearch.SearchBound(center?.toLatLonPoint(), range?:100)
    }
